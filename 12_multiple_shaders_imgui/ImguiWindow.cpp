...
// ==== BoxWindow =====================================================================================================
ImGui::BoxWindow::BoxWindow(bool& _show) : BaseWindow(_show), boxShaderProgram(gloo::Shader(gloo::getFileContents(SRC_ROOT "/12_multiple_shaders_imgui/shaders/plane.vert"), gloo::Shader::Type::Vertex), gloo::Shader(gloo::getFileContents(SRC_ROOT "/12_multiple_shaders_imgui/shaders/box.frag"), gloo::Shader::Type::Fragment)), screenMesh(std::vector<gloo::Vertex>(SCREEN_VERTICES), std::vector<GLuint>(SCREEN_INDICES)), viewSize(640,480), viewWindowSize(viewSize.x+14,viewSize.y+14), in_color1(0.90,0.60,0.30), in_color2(0.65,0.85,1.00), in_box_center(0.5,0.5), in_box_sides(0.25,0.25), renderTex(viewSize.x, viewSize.y, gloo::Texture::Type::UnsignedByte, gloo::Texture::InternalFormat::RGBA, gloo::Texture::Format::RGBA, gloo::Texture::Slot::slot00, gloo::Texture::Target::Texture2D) {
  screenMesh.LinkPositionToLocation(0);
  screenMesh.LinkTextureUVToLocation(1);
  gloo::Framebuffer::UNBIND();
}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void ImGui::BoxWindow::Initialize(AppData& _appData) {
  boxShaderProgram.Uniform("in_aspectRatio", renderTex.aspectRatio);
  boxShaderProgram.Uniform("in_sides", in_box_sides);
  boxShaderProgram.Uniform("in_center", in_box_center);
  boxShaderProgram.Uniform("in_time", (float)ImGui::GetTime());
  boxShaderProgram.Uniform("in_color1", in_color1);
  boxShaderProgram.Uniform("in_color2", in_color2);
  framebuffer.Bind();
  renderbuffer.RenderToTexture(renderTex);
  gloo::Framebuffer::UNBIND();
}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void ImGui::BoxWindow::Draw(AppData& _appData) {
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void ImGui::BoxWindow::Deinitialize(AppData& _appData) {
}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ImGui::BoxWindow::~BoxWindow() {
...
// ==== CircleWindow ==================================================================================================
ImGui::CircleWindow::CircleWindow(bool& _show) : BaseWindow(_show), circleShaderProgram(gloo::Shader(gloo::getFileContents(SRC_ROOT "/12_multiple_shaders_imgui/shaders/plane.vert"), gloo::Shader::Type::Vertex), gloo::Shader(gloo::getFileContents(SRC_ROOT "/12_multiple_shaders_imgui/shaders/circle.frag"), gloo::Shader::Type::Fragment)), screenMesh(std::vector<gloo::Vertex>(SCREEN_VERTICES), std::vector<GLuint>(SCREEN_INDICES)), viewSize(480,480), viewWindowSize(viewSize.x+14,viewSize.y+14), in_color1(0.90,0.60,0.30), in_color2(0.65,0.85,1.00), in_circle_center(0.5,0.5), in_circle_radius(0.25), renderTex(viewSize.x, viewSize.y, gloo::Texture::Type::UnsignedByte, gloo::Texture::InternalFormat::RGBA, gloo::Texture::Format::RGBA, gloo::Texture::Slot::slot01, gloo::Texture::Target::Texture2D) {
  screenMesh.LinkPositionToLocation(0);
  screenMesh.LinkTextureUVToLocation(1);
  gloo::Framebuffer::UNBIND();
}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void ImGui::CircleWindow::Initialize(AppData& _appData) {
  circleShaderProgram.Uniform("in_aspectRatio", renderTex.aspectRatio);
  circleShaderProgram.Uniform("in_radius", in_circle_radius);
  circleShaderProgram.Uniform("in_center", in_circle_center);
  circleShaderProgram.Uniform("in_time", (float)ImGui::GetTime());
  circleShaderProgram.Uniform("in_color1", in_color1);
  circleShaderProgram.Uniform("in_color2", in_color2);
  framebuffer.Bind();
  renderbuffer.RenderToTexture(renderTex);
  gloo::Framebuffer::UNBIND();
}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void ImGui::CircleWindow::Draw(AppData& _appData) {
  static float zoom=1.0, zoomMin=1.0, zoomMax=8.0;
  float circle_center[2] = {in_circle_center.x, in_circle_center.y};
  ImVec4 color1 = {in_color1.x, in_color1.y, in_color1.z, 1.0};
  ImVec4 color2 = {in_color2.x, in_color2.y, in_color2.z, 1.0};
  ...
  ImGui::SameLine();
  ImGui::SetNextItemWidth(300);
  ImGui::SliderFloat("##circle_radius", &in_circle_radius, 0.0f, 1.0f, "%.3f");
  // ---- radius slider -----------------------------------------------------------------------------------------------
  ...
  ImGui::SameLine();
  ImGui::SetNextItemWidth(300);
  if (ImGui::SliderFloat2("##circle_center", circle_center, 0.0f, 1.0f)) {
  ...
  ImGui::SameLine();
  ImGui::SetNextItemWidth(300);
  ImGui::SliderScalar("##circle_zoom", ImGuiDataType_Float, &zoom, &zoomMin, &zoomMax, "%.2f X");
  // ---- zoom slider -------------------------------------------------------------------------------------------------
  ...
  ImGui::Text("Colors    ");
  ImGui::SameLine();
  if (ImGui::ColorEdit4("##circle_color1", (float*)&color1, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_PickerHueBar)) {
    in_color1.x = color1.x;
    in_color1.y = color1.y;
    in_color1.z = color1.z;
  }
  ImGui::SameLine();
  if (ImGui::ColorEdit4("##circle_color2", (float*)&color2, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoAlpha | ImGuiColorEditFlags_NoSidePreview | ImGuiColorEditFlags_PickerHueBar)) {
    in_color2.x = color2.x;
    in_color2.y = color2.y;
    in_color2.z = color2.z;
  }
  ...
  ImGui::PushStyleVar(ImGuiStyleVar_ChildRounding, 5.0f);
  ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(6,6));
  ImGui::BeginChild("circle_tex_view", viewWindowSize, true, ImGuiWindowFlags_None | ImGuiWindowFlags_HorizontalScrollbar);
  renderTex.Update();
  ImGui::Image((ImTextureID)(GLuint)renderTex, viewSize);
  ImGui::EndChild();
  ImGui::PopStyleVar();
...
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
void ImGui::CircleWindow::Deinitialize(AppData& _appData) {
}
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ImGui::CircleWindow::~CircleWindow() {
  // circleShaderProgram.Delete();
  // screenMesh.Delete();
  // renderTex.Delete();
}
// ==== CircleWindow ==================================================================================================
