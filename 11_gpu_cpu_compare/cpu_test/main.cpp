...
int main()
{
    ...
    // GL 3.0 + GLSL 130
    glfw::Window window(windowWidth, windowHeight, "11 CPU Test");
    ...
    gloo::Program screenShaderProgram(gloo::Shader(gloo::getFileContents(SRC_ROOT "/11_gpu_cpu_compare/cpu_test/direct.vert"), gloo::Shader::Type::Vertex), gloo::Shader(gloo::getFileContents(SRC_ROOT "/11_gpu_cpu_compare/cpu_test/direct.frag"), gloo::Shader::Type::Fragment));
    ...
    unsigned char *gridTexBytes = stbi_load(SRC_ROOT "/11_gpu_cpu_compare/cpu_test/grid.png", &texWidth, &texHeight, &_gridTexNCh, 0);
    ...
    static glm::vec2 circle_center(0.5, 0.5);
    static float circle_radius = 0.25;
    // ------------------------------------------------------------------------------------------------------------------
    uint8_t *in_gradTexBytes = new uint8_t[texWidth * texHeight]();
    for (int y = 0; y < texHeight; y++)
        for (int x = 0; x < texWidth; x++)
            in_gradTexBytes[x + y * texWidth] = (uint8_t)pow(2.0, 16) * (float)x / (float)texWidth;
    gloo::Texture in_gradTex(in_gradTexBytes, texWidth, texHeight, gloo::Texture::Type::UnsignedByte, gloo::Texture::InternalFormat::Red, gloo::Texture::Format::Red, gloo::Texture::Slot::slot01, gloo::Texture::Target::Texture2D);
    // ------------------------------------------------------------------------------------------------------------------
    float norm_x2, norm_y2, norm_radius;
    uint8_t *in_maskTexBytes = new uint8_t[texWidth * texHeight]();
    for (int y = 0; y < texHeight; y++)
    {
        for (int x = 0; x < texWidth; x++)
        {
            norm_x2 = pow((float)x / (float)texWidth - circle_center.x, 2);
            norm_y2 = pow((float)y / (float)texWidth - circle_center.y * (float)texHeight / (float)texWidth, 2);
            norm_radius = sqrt(norm_x2 + norm_y2);
            in_maskTexBytes[x + y * texWidth] = (unsigned char)pow(2.0, 8) * ((norm_radius > circle_radius) ? 0 : 1);
        }
    }
    gloo::Texture in_maskTex(in_maskTexBytes, texWidth, texHeight, gloo::Texture::Type::UnsignedByte, gloo::Texture::InternalFormat::Red, gloo::Texture::Format::Red, gloo::Texture::Slot::slot02, gloo::Texture::Target::Texture2D);
    // ------------------------------------------------------------------------------------------------------------------
    screenShaderProgram.Uniform("in_image", in_gridTex);
    screenShaderProgram.Uniform("in_gradient", in_gradTex);
    screenShaderProgram.Uniform("in_mask", in_maskTex);
    // ------------------------------------------------------------------------------------------------------------------
    size_t iStop_1 = 0;
    ...
    while (!window.ShouldClose())
    {
        ...
        circle_center = getPosition(time_s.count());
        circle_radius = getRadius(time_s.count());
        ...
        for (int y = 0; y < texHeight; y++)
        {
            for (int x = 0; x < texWidth; x++)
            {
                ...
                norm_y2 = pow((float)y / (float)texWidth - circle_center.y * (float)texHeight / (float)texWidth, 2);
                norm_radius = sqrt(norm_x2 + norm_y2);
                in_maskTexBytes[x + y * texWidth] = (uint8_t)pow(2.0, 8) * ((norm_radius > circle_radius) ? 0 : 1);
            }
        }
        in_maskTex.Update(in_maskTexBytes);
        screenMesh.Draw();
        // ----------------------------------------------------------------------------------------------------------------
    ...
    delete in_gradTexBytes;

    delete in_maskTexBytes;
